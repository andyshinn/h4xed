...
class Playlist extends Controller
{
    ...
    {        
        ...
        $tmpl = array(
            'table_open'=>'<table class="requests">', 'heading_row_start'=>'<tr>', 'heading_row_end'=>'</tr>', 'heading_cell_start'=>'<th>', 'heading_cell_end'=>'</th>', 'row_start'=>'<tr>', 'row_end'=>'</tr>', 'cell_start'=>'<td>', 'cell_end'=>'</td>', 'row_alt_start'=>'<tr class="alt">', 'row_alt_end'=>'</tr>', 'cell_alt_start'=>'<td>', 'cell_alt_end'=>'</td>', 'table_close'=>'</table>'
        );
        ...
        for ($day = 0; $day < $days; $day++)
        {
			...
			$this->template->write_view('main', 'public/requests', $this->view_data);
    ...
    function artists($letter = 'all', $limit_offset = 0)
    {
        ...
        foreach ($artist as $key=>$value)
        {
            ...
            $artist_url[$key] = array_merge($value, array(
                'artist_url'=>reduce_multiples(url_title(strtolower(trim($artist_new_name))))
            ), '-');
        }
    ...
    function artist($artist_safe = null, $album_safe = null)
    {
        if ($artist_safe)
        {
            ...
            foreach ($albums_urls as $album)
            {
                ...
                $this->template->parse_view('main', 'public/tracks', array(
                    'artist_name'=>$artist->artist, 'tracks'=>$tracks_new, 'album_name'=>$album['album_name'], 'album_url'=>$album['album_url']
                ));
            }
    ...
    function _build_array($data, $prefix = 'album')
    {
        ...
        {
            $array[$key] = array(
                $prefix.'_url'=>$this->_safe_search($value[$prefix.'_name'])
            );
        }
    ...
    function show($letter = 'all', $limit_offset = 0)
    {
        ...
        $popup_attributes = array(
            'width'=>'500', 'height'=>'300', 'scrollbars'=>'no', 'status'=>'no', 'resizable'=>'yes', 'screenx'=>'100', 'screeny'=>'100'
        );

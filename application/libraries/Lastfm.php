...
class Lastfm {
	...
	function artist_getInfo($artist) {
		...
		$request = array(array(array('api_key' => $this->api_key, 'artist' => str_replace('_', ' ', $artist)), 'struct'));
	...
	function request($method = null, $params = array()) {
	...
	function handshake() {
		...
		$request_array = array(// Indicates that a handshake is requested. Requests without this parameter set to true will return a human-readable informational message and no handshake will be performed.
'hs' => 'true', 
	...
	function nowplaying($artist, $track, $album, $sec, $track_number, $mb_trackid = '') {
		...
		$request_array = array('s' => $this->SessionId, 'a' => $artist, 't' => $track, 'b' => $album, 'l' => $sec, 'n' => $track_number, 'm' => $mb_trackid);
	...
	function add($artist, $track, $time, $source, $rating, $secs, $album = '', $track_number = '', $mb_trackid = '') {
		...
		$this->arraysongs[] = array('a' => $artist, 't' => $track, //        'i'=>$this->GetTime_SubmitFormat($time[0], $time[1], $time[2], $time[3], $time[4], $time[5]),
'i' => $time, 'o' => $source, 'r' => $rating, 'l' => $secs, 'b' => $album, 'n' => $track_number, 'm' => $mb_trackid);
	}
